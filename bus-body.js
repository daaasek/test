function GDataBusinessMainpageOverview() {
    var i = this;
    this.DOMElements = {
        categoryItemLinks: document.querySelectorAll(".category__item > h2 > a")
    },
    this.init = function() {
        i._setCategoryItemLink()
    }
    ,
    this._setCategoryItemLink = function() {
        for (var e = function _loop(e) {
            var t = i.DOMElements.categoryItemLinks[e]
              , n = t.parentNode.parentNode;
            n.addEventListener("click", function(e) {
                e.preventDefault(),
                "_blank" === t.target ? window.open(t.href) : window.location = t.href
            })
        }, t = 0; t < i.DOMElements.categoryItemLinks.length; t++)
            e(t)
    }
}
var GdWebsite = GdWebsite || {};
GdWebsite.BusinessMainpageOverview = new GDataBusinessMainpageOverview,
GdWebsite.BusinessMainpageOverview.init();
...
function hideSideStoerer() {
    if (window.gdataSideStoererIsActive == 1) {
        $('#side-stoerer-outer').fadeOut(500, function() {
            $('#side-stoerer').remove();
        });
    }
}

function showSideStoerer() {
    $('#side-stoerer').css('display', 'table');
    $('#side-stoerer').css('visibility', 'visible');
    setTimeout(function() {
        $('#side-stoerer').css('opacity', 1);
    }, 1000);
    sessionStorage.setItem(window.gdataSideStoererName, parseInt(sessionStorage.getItem(window.gdataSideStoererName)) + 1);
    $('[data-side-stoerer-close]').click(function(e) {
        e.preventDefault();
        hideSideStoerer();
    });
    $('.close-of-click').click(function(e) {
        hideSideStoerer();
    });
    if (window.gdataSideStoererHos == 1) {
        $(window).scroll(function() {
            if (window.gdataSideStoererIsActive == 1) {
                hideSideStoerer();
            }
    ...
    if (window.gdataSideStoererTimeout > 0) {
        window.setTimeout(hideSideStoerer, window.gdataSideStoererTimeout);
    }
}

$(function() {
    if (sessionStorage.getItem(window.gdataSideStoererName) != parseInt(sessionStorage.getItem(window.gdataSideStoererName))) {
        sessionStorage.setItem(window.gdataSideStoererName, 0);
    }
    if (window.gdataSideStoererShowMax == 0 || parseInt(sessionStorage.getItem(window.gdataSideStoererName)) < window.gdataSideStoererShowMax) {

        setTimeout(function() {
            showSideStoerer();
        }, parseInt(window.gdataSideStoererShowTimer * 1000));
    } else {
        $('#side-stoerer').remove();
    }
...
0 < $("#sticky-nav").length && $("#sticky-nav-btn").click(function(s) {
    s.preventDefault(),
    $("#sticky-nav").hasClass("show-menu") ? $("#sticky-nav").removeClass("show-menu") : $("#sticky-nav").addClass("show-menu")
}),
$(window).resize(function() {
    0 < $("#sticky-nav").length && ($("#sticky-nav-wrapper").height($("#sticky-nav").height()),
    window.stickyNavOffsetTop = $("#sticky-nav").offset().top)
});
